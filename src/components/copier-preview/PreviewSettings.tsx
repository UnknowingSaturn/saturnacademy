...
import { CopierStatus } from "@/types/copier-preview";
...
export function PreviewSettings({ status }: PreviewSettingsProps) {
  ...
  const handleSaveApiKey = () => {
    ...
    setTimeout(() => {
      ...
      toast.success("API Key Saved", {
        description: "Configuration synced from cloud",
      });
      setTimeout(() => setSaved(false), 2000);
    }, 1000);
  };

  const handleSetMt5Path = () => {
    if (!mt5Path.trim()) return;
    toast.success("MT5 Path Updated", {
      description: "Path has been set successfully",
    });
  };
  ...
  const handleInstallUpdate = () => {
    ...
    toast.info("Installing Update", {
      description: "This would restart the app in the desktop version",
    });
    
    setTimeout(() => {
      setInstalling(false);
      toast.success("Update Installed", {
        description: "App would restart now",
      });
    }, 2000);
  };
