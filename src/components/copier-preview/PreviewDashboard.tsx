...
import { CopierStatus, Execution, Mt5Terminal, MasterHeartbeat } from "@/types/copier-preview";
...
export function PreviewDashboard({
  ...
  const handleSync = () => {
    toast.success("Configuration synced", {
      description: "Simulated sync from cloud configuration",
    });
  };

  const handleStart = () => {
    toast.success("Copier started", {
      description: "Trade copier is now running",
    });
  };

  const handleStop = () => {
    toast.info("Copier stopped", {
      description: "Trade copier has been stopped",
    });
  };
