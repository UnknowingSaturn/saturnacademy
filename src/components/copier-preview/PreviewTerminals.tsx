...
interface PreviewTerminalsProps {
  terminals: Mt5Terminal[];
}

export function PreviewTerminals({ terminals }: PreviewTerminalsProps) {
  const [installing, setInstalling] = useState<string | null>(null);

  const installEa = (terminalId: string, eaType: "master" | "receiver") => {
    setInstalling(`${terminalId}-${eaType}`);
    
    setTimeout(() => {
      setInstalling(null);
      toast.success(`${eaType === 'master' ? 'Master' : 'Receiver'} EA Installed`, {
        description: "EA installed successfully. Refresh MT5 Navigator to see it.",
      });
    }, 1500);
  };
  ...
  const handleRefresh = () => {
    toast.info("Scanning Terminals", {
      description: "Looking for MT5 installations...",
    });
  };
