...
export const AIAnalysisProgress = forwardRef<HTMLDivElement, AIAnalysisProgressProps>(
  function AIAnalysisProgress({ progress, isAnalyzing, onRetry }, ref) {
    ...
    return (
      <div ref={ref} className="space-y-4 p-4 border border-border rounded-lg bg-card">
        {/* Progress bar */}
        <div className="space-y-2">
          <div className="flex justify-between text-xs text-muted-foreground">
            <span>{step === "complete" ? "Loading results..." : message}</span>
        ...
        {/* Success state - show loading message until data arrives */}
        {step === "complete" && (
          <div className="flex items-center gap-2 pt-2 border-t border-border text-profit">
            <Loader2 className="w-4 h-4 animate-spin" />
            <span className="text-sm font-medium">Loading analysis results...</span>
          </div>
        )}
      </div>
    );
