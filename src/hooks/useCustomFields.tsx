...
// (formerly src/hooks/useFieldOverrides.tsx — folded in 2026-05-28)
//
// Overrides are stored as rows in `custom_field_definitions` where
// scope='system_override' and key=field_key. Wire/UI shape preserved.
// =====================================================================
...
const transformOverride = (row: any): FieldOverride => ({
  id: row.id,
  user_id: row.user_id,
  field_key: row.key,
  type: row.type as CustomFieldType,
...
export function useFieldOverrides() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (error) throw error;
      return ((data || []) as any[]).map(transformOverride);
    },
    enabled: !!user?.id,
  });
}
