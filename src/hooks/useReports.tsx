...
export function useReports(trades: Trade[], period: ReportPeriod) {
  ...
  const metrics: ReportMetrics = useMemo(() => {
    if (filteredTrades.length === 0) {
      return {
        totalTrades: 0, winRate: 0, profitFactor: 0, totalPnl: 0, avgRMultiple: 0,
        bestTrade: null, worstTrade: null, avgWin: 0, avgLoss: 0, largestWin: 0, largestLoss: 0,
        consecutiveWins: 0, consecutiveLosses: 0, tradesBySymbol: {}, tradesBySession: {},
        tradingDays: 0, avgTradesPerDay: 0,
      };
    }
    ...
    const rMultiples = filteredTrades.filter(t => t.r_multiple_actual !== null).map(t => t.r_multiple_actual!);
    ...
    filteredTrades.forEach(t => {
      if (!tradesBySymbol[t.symbol]) tradesBySymbol[t.symbol] = { count: 0, pnl: 0 };
    ...
    filteredTrades.forEach(t => {
      ...
      if (!tradesBySession[session]) tradesBySession[session] = { count: 0, pnl: 0 };
    ...
    return {
      ...
      tradesBySymbol, tradesBySession,
...
export function getWeekPeriod(date: Date): ReportPeriod {
  ...
  return { type: 'week', start, end, label: `Week of ${format(start, 'MMM d')} - ${format(end, 'MMM d, yyyy')}` };
}

export function getMonthPeriod(date: Date): ReportPeriod {
  const start = startOfMonth(date);
  const end = endOfMonth(date);
  return { type: 'month', start, end, label: format(start, 'MMMM yyyy') };
}
