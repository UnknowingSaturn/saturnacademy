...
-- 1) Trade tier: is this trade actually journaled?
CREATE OR REPLACE VIEW public.journal_trade_tier
WITH (security_invoker = on) AS
SELECT
  t.id                      AS trade_id,
  t.user_id,
  CASE
    WHEN COALESCE(t.actual_playbook_id, t.playbook_id) IS NOT NULL AND rv.trade_id IS NOT NULL THEN 'journaled'
    WHEN COALESCE(t.actual_playbook_id, t.playbook_id) IS NOT NULL OR  rv.trade_id IS NOT NULL THEN 'partial'
    ELSE 'raw'
  END AS tier
FROM public.trades t
LEFT JOIN LATERAL (
  SELECT tr.trade_id FROM public.trade_reviews tr WHERE tr.trade_id = t.id LIMIT 1
) rv ON true;

GRANT SELECT ON public.journal_trade_tier TO authenticated;
GRANT SELECT ON public.journal_trade_tier TO service_role;

-- 2) One cohort engine. Every Coach statistic comes from here.
...
DECLARE
  ...
  v_n integer;
BEGIN
  CREATE TEMP TABLE IF NOT EXISTS _jc_base (LIKE public.trades) ON COMMIT DROP;

  -- population: all filters EXCEPT tier (so coverage can report what tier excludes)
  WITH pop AS (
  ...
  cohort AS (
    SELECT * FROM pop
    WHERE _tiers IS NULL OR tier = ANY(_tiers)
  ),
  agg AS (
    SELECT
      count(*)::int AS n,
  ...
  cov AS (
    SELECT jsonb_object_agg(tier, jsonb_build_object(
             'n', n, 'net_pnl', pnl, 'win_rate_pct', wr, 'expectancy_r', er))
    FROM (
  ...
  grp AS (
    SELECT jsonb_agg(g ORDER BY (g->>'expectancy_r')::numeric DESC NULLS LAST)
    FROM (
      ...
      FROM (
        SELECT c.*, CASE _group_by
          ...
          WHEN 'weekday'   THEN to_char(c.entry_time, 'Day')
  ...
  SELECT to_jsonb(agg.*), (SELECT * FROM cov), (SELECT * FROM grp)
  INTO v_stats, v_coverage, v_groups
  FROM agg;

  v_n := COALESCE((v_stats->>'n')::int, 0);

  -- Immutable, pre-rendered fact lines. The model may quote these verbatim and
  -- must not compute its own numbers from raw rows.
  SELECT jsonb_agg(f ORDER BY ord) INTO v_facts FROM (
    ...
    UNION ALL
    SELECT 3 + row_number() OVER (), jsonb_build_object(
...
GRANT EXECUTE ON FUNCTION public.journal_cohort(uuid, text[], text, text, text, text, timestamptz, timestamptz, integer, uuid[], text, boolean) TO service_role;
GRANT EXECUTE ON FUNCTION public.journal_cohort(uuid, text[], text, text, text, text, timestamptz, timestamptz, integer, uuid[], text, boolean) TO authenticated;
